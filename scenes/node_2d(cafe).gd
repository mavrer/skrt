...
@onready var drink = $Points/SpawnQueue
...
func get_drink_position(name: String) -> Vector2:
...
func _on_global_timer_timeout():
	...
	for npc in npc_container.get_children():
		if npc.finished_drinking:
			npc.move_to_exit()

func spawn_customer(npc_data):
	var new_npc = preload("res://scenes/customer.tscn").instantiate()
	new_npc.npc_name = npc_data["name"]
